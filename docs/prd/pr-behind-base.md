...
|---|---|---|---|---|
| R-001 | **Resolved, then corrected during implementation 2026-09-18.** `Ref.compare(headRef: String!): Comparison` exists, and the fork-qualified `owner:branch` head ref works. Two things the introspection did not catch, both since measured against the live API: (a) the argument is **static per selection**, so a bulk search over 100 PRs cannot name each node's own head ref -- the compare cannot ride inside `PR_SEARCH_GRAPHQL`, and the zero-extra-requests guarantee does **not** hold as written (see R-005 and section 13); (b) **superseded -- see R-006.** This row previously recorded that the field to read was `aheadBy`. That was wrong, and it shipped in #127. | Risk | Implementer | Closed with corrections |
...
| R-006 | **Correction to R-001(b), found in production validation 2026-09-18 after #127 merged.** `Ref.compare(headRef:)` describes the **head ref relative to the receiver**, exactly like the REST endpoint `/repos/:o/:r/compare/<base>...<head>`. So on `baseRef.compare(headRef: <head>)`, **`behindBy`** is how far the head is behind the base -- the number this lane is about -- and `aheadBy` is the PR's own commit count. Ground truth, both re-runnable: REST `waf main...feat/remove-password-path` -> `{ahead_by: 4, behind_by: 9}` and the identical GraphQL compare -> `{aheadBy: 4, behindBy: 9}`; and a revision four commits behind `main` with nothing of its own -> `{aheadBy: 0, behindBy: 4, status: BEHIND}`, matching `git rev-list --count`. Reading `aheadBy` made the lane flag every PR with any commits of its own and never flag a stale one. Covered from now on by SC-detect-compare-field. | Defect | Implementer | Fixed |
...
- **Integrations:**
  - **GitHub GraphQL** -- detection. `PR_SEARCH_GRAPHQL` and `PR_BY_NUMBER_GRAPHQL` gain
    ...
    `baseRef { compare(headRef: $hN) { behindBy } }` (see R-001 for why it is not inside the
    search query, and R-006 for the field name). Auth is the existing App-installation or PAT
