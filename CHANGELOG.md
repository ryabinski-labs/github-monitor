...
- Cancelled workflow runs now arrive pre-dismissed. A cancelled run offers no retry worth queueing and no failure to read — most are the concurrency group cancelling the previous commit's run — so it leaves the actionable Failing CI list and its tile while staying reachable behind the dismissed bar's "Show", labelled `Auto-dismissed`. User dismissals are untouched, so `Restore all` never drags them back. Set `AUTO_DISMISS_CANCELLED_RUNS=0` to opt out. Cancelled *CD* runs are deliberately excluded: a cancelled deploy means the change never reached production.
