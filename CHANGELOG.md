...
- **Running, Finished and Failed CD** rows give the title the row's flexible space instead of a sliver. The row's flexible track went to the short "Deploy #1304" meta, so a long title clamped hundreds of pixels early while the middle of the row sat empty, and the branch-and-time meta wrapped `AM` onto a third line. The run-number meta now sits at its natural width, the branch and time stay on one line -- a long branch ellipsises instead -- and a running status reads `In progress` instead of the raw `in_progress` enum.
